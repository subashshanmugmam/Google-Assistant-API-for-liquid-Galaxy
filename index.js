...
app.intent('FlySensors', (conv) => {
  ...
  axios.post(url + '/assistant/move', { name: conv.parameters['sensor'] })
    .catch(error => {
      handleError(conv, error);
    })
    .then(r => {
      return axios.post(url + '/assistant/opensite', { name: process.env.sensor });
    })
    .then(r => {
      return axios.post(url + '/assistant/openballon', { name: process.env.sensor });
    })
    .catch(error => {
      handleError(conv, error);
    });
...
app.intent('Where', function (conv) {
  ...
  return axios.get(url)
    .then(function (res) {
      ...
      response = new SimpleResponse({
        text: "you are in: " + res.data.results[0].formatted,
        speech: "you are in: " + res.data.results[0].formatted
      });
      conv.ask(response);
    })
    .catch(function (err) {
